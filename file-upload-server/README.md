...
bash
node server.js
Test the API – Send a POST request to http://localhost:8000/api/upload with a file field named file
...
LIMIT_FILE_SIZE – Returns 413 status with message "File too large. Maximum size is 5MB."

Other Multer errors – Returns 400 status with error message

General server errors – Returns 500 status
...
During the development of this file upload server, I faced several challenges that I had to work through. The first problem arose from configuring Multer storage correctly so that files would be uploaded to the proper directory, which was not happening due to incorrectly configured paths. I resolved this by carefully setting the destination path in the disk storage configuration and ensuring the uploads directory existed before the server started. Another challenge related to validating the file type based on MIME type, because initially, the file filter did not prevent unwanted file types from being uploaded. I fixed this by implementing a proper fileFilter function that checks the mimetype of incoming files and only allows image/jpeg, image/png, and application/pdf, while rejecting others with a clear error message. Additionally, handling Multer-specific errors like LIMIT_FILE_SIZE required creating a dedicated error-handling middleware that checks the error instance and returns appropriate HTTP status codes (413 for file too large). Finally, configuring CORS to allow the frontend running on port 3000 to communicate with the backend on port 8000 was initially problematic, but I solved it by setting the origin option in the CORS middleware to read from the .env file. Through debugging and systematic testing, I managed to overcome all these difficulties and develop a reliable back-end file upload system.
...
What I Learned
In this project, I successfully created a backend file upload feature using Node.js and Express.js by implementing an endpoint at /api/upload. I learned how to use Multer middleware to process multipart/form-data requests, save uploaded files to a specific directory, and rename files to ensure no duplicates override one another. I also implemented server-side file validation to allow only specific file types (JPEG, PNG, and PDF) and enforce a 5MB size limit. I gained practical experience in configuring CORS for cross-origin communication, creating specialized error-handling middleware for Multer errors, and serving static files using Express. Despite the difficulties involved, debugging and testing enabled me to develop a reliable back-end system. This project helped me learn more about managing files on the server side, configuring middleware, and implementing secure file uploads using Node.js.